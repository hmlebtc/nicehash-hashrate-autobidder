...
| Price unit | `sat / EH / day` | `BTC / <display-unit> / day` (PH/s for SHA256), scaled by `marketFactor` |
...
| `daemon/src/config/schema.ts` | New fields: NiceHash `apiKey`/`apiSecret`/`orgId`, `market`, `algorithm`, `refill_amount_btc`, `refill_when_runway_hours`, `min_order_amount_btc`. Prices expressed in BTC/unit/day. (Market + algorithm are configurable, no hardcoded default — per the project requirement.) |
