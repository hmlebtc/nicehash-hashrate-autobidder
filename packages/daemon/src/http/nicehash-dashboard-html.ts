...
(function () {
  ...
  var CFG = [
    ...
    { group: 'Track-to-fill', items: [
      ['anchorNextFilledTier', 'Anchor on next filled tier', 'checkbox', 'Track the next filled tier (the cyan line — the bottom of the contiguously filled top of the book, with no zero-miner orders above it) instead of the marginal (purple, the cheapest fill). On a lumpy book a bid a hair above the marginal often wins nothing because the market is really clearing into the block far above; anchoring at that block\'s bottom + your overpay puts the bid where fills provably land (still clamped by the cap). Falls back to the marginal when the filled top reaches the marginal itself. On is recommended.'],
